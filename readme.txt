...
Note: if you are interested in using command-line arguments, see addendum below.
...
Addendum to Step 2:
To use command-line arguments, well it's pretty hard to explain, here's an example.
I want to run Paint and have it open c:\test.bmp. I also want to run Firefox and have it open Google. Thirdly, I want to run a dummy test program that i wrote that just prints out all the arguments passed to it, with the arguments "This program", "is", "really", and "cool". And finally, I want to run the Calculator. So, here is the shortcut target (all on one line):

"c:\multishort.exe" "c:\windows\system32\mspaint.exe c:\test.bmp" "c:\program files\mozilla firefox\firefox.exe http://www.google.com" "c:\progs\args.exe \"This program\" is really cool." c:\windows\system32\calc.exe

So to pass command-line arguments, simply put them after the program name/path and enclose the whole thing in quotes.
Notice how everything with command-line arguments is in quotes. The old rules about spaces in the program paths and quotation marks still apply, with the addition that if you are passing command-line arguments, you must use quotes.
Also notice \"This program\". To pass This and program as one arguments, you must enclose them in quotes (normally arguments are separated by spaces, unless when they are in quotes). But, the whole call to args.exe is already in quotes! What do we do? The answer is that all "s must have backslashes before them. Period. Otherwise stuff will get messed up. DO NOT FORGET THE RULES CONCERNING QUOTES. You have been warned.
I know this is rather confusing; if you can't figure out how to construct a particular shortcut, email me at multishort@hotmail.com with the programs you want to run and i'll make the shortcut target, or the whole file, for you (note that I don't know if the shortcut mechanism has changed for XP so if you don't have XP shortcut files I make for you are not guaranteed to work).
...
However, nothing prevents MultiShort from running programs that are spy/ad/malware/viruses, and there is nothing i can do to prevent this. Therefore it is not my fault. If someone else gives you a shortcut target for MultiShort, check the file paths to make sure it is not running anything suspicious. You have been warned.
